    ...
    const init = async () => {
      ...
      if (letterSpacing !== 0) {
        for (const char of text) {
          totalWidth += measureCtx.measureText(char).width + letterSpacing;
        }
        totalWidth -= letterSpacing;
      } else {
        totalWidth = measureCtx.measureText(text).width;
      }
      ...
      function renderOffscreen(fillColor) {
        const oc = document.createElement('canvas');
        const octx = oc.getContext('2d');
        oc.width = offscreenWidth;
        oc.height = tightHeight;
        octx.font = `${fontWeight} ${fontSizeStr} ${computedFontFamily}`;
        octx.textBaseline = 'alphabetic';

        if (gradient && Array.isArray(gradient) && gradient.length >= 2) {
          const grad = octx.createLinearGradient(0, 0, offscreenWidth, 0);
        ...
        if (letterSpacing !== 0) {
          let xPos = xOffset;
          for (const char of text) {
            octx.fillText(char, xPos, actualAscent);
            xPos += octx.measureText(char).width + letterSpacing;
          }
        } else {
          octx.fillText(text, xOffset - actualLeft, actualAscent);
        }
        return oc;
      }

      const offscreen = renderOffscreen(color);
      const offscreenHover = hoverColor ? renderOffscreen(hoverColor) : offscreen;
      ...
      const run = timestamp => {
        ...
        const activeOffscreen = isHovering && hoverColor ? offscreenHover : offscreen;

        if (direction === 'horizontal') {
          for (let j = 0; j < tightHeight; j++) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(activeOffscreen, 0, j, offscreenWidth, 1, dx, j, offscreenWidth, 1);
          }
        } else if (direction === 'vertical') {
          for (let i = 0; i < offscreenWidth; i++) {
            const dy = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(activeOffscreen, i, 0, 1, tightHeight, i, dy, 1, tightHeight);
          }
        } else {
          for (let j = 0; j < tightHeight; j++) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
